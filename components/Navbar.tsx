...
const Navbar: React.FC = () => {
  ...
  return (
    <>
      <div className="fixed top-6 left-0 right-0 z-50 flex justify-center pointer-events-none px-4 md:px-0">
        <nav 
            className={`pointer-events-auto transition-all duration-500 ease-out border ${
                ...
                : 'w-full bg-transparent border-transparent px-0 py-4'
            }`}
        >
          <div className="flex items-center justify-between md:justify-center md:gap-8 px-4">
            
            <a href="#" className={`text-xl font-display font-bold tracking-tighter transition-colors uppercase ${scrolled ? 'mr-4' : 'mr-auto'}`}>
            ...
            <div className="hidden md:flex items-center bg-gray-100 dark:bg-[#111] rounded-full px-1 py-1 border border-gray-200 dark:border-gray-800">
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  className="relative px-5 py-2 text-xs font-bold uppercase tracking-wider hover:text-design-blue transition-colors group overflow-hidden"
                >
            ...
            <div className={`flex items-center gap-2 ${scrolled ? 'ml-4' : 'ml-auto'}`}>
               <button 
                  onClick={toggleTheme}
                  className="w-10 h-10 rounded-full bg-gray-100 dark:bg-[#111] flex items-center justify-center border border-gray-200 dark:border-gray-800 hover:border-design-blue transition-colors text-design-black dark:text-white"
               >
               ...
               <button 
                  ...
                  className="md:hidden w-10 h-10 rounded-full bg-design-black dark:bg-white text-white dark:text-black flex items-center justify-center border border-transparent dark:border-white/10"
               >
                  <Menu size={18} />
               </button>
            </div>
